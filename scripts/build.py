...
function batterMatchupCell(opp) {{
  const bm = getBatterMatchup(opp);
  if (!bm.pitcher) return '<span class="mup-avg">?</span>';
  const lastName = bm.pitcher.FullName.split(' ').pop();
  const hand = bm.pitcher.PitcherHand;
  return '<span class="' + bm.cls + '">' + bm.label + '</span> <span style="font-size:.62rem;color:var(--text2)">' + lastName + '(' + hand + ')</span>';
}}
...
function rB(){{
  ...
  document.getElementById('btb').innerHTML=d.map(b=>{{const sc=bScore(b).toFixed(1);const rc=rowClass(b.FullName);const bw=((b.Hits/mH)*40).toFixed(0);return`<tr class="${{rc}}"><td class="sc">${{sc}}</td><td style="font-weight:500;white-space:nowrap">${{b.FullName}}</td><td>${{statusTag(b.FullName)}}</td><td><span class="tb">${{b.Team}}</span></td><td style="color:var(--text2);font-size:.72rem">vs ${{b.Opponent}}</td><td>${{batterMatchupCell(b.Opponent)}}</td><td><span class="pb">${{b.BattingPosition}}</span></td><td><div style="display:flex;align-items:center;gap:4px"><div style="height:3px;border-radius:2px;background:var(--accent);width:${{bw}}px;min-width:2px"></div><span class="sv ${{cs(b.Hits,.6,.8,1)}}">${{b.Hits.toFixed(3)}}</span></div></td><td class="sv ${{cs(b.Runs,.3,.45,.55)}}">${{b.Runs.toFixed(3)}}</td><td class="sv ${{cs(b.HomeRuns,.08,.13,.18)}}">${{b.HomeRuns.toFixed(3)}}</td><td class="sv ${{cs(b.TB,1,1.4,1.7)}}">${{b.TB.toFixed(3)}}</td><td class="sv ${{cs(b.SB,.01,.05,.12)}}">${{b.SB.toFixed(3)}}</td><td class="sv ${{cs(b.OBP,.28,.33,.38)}}">${{b.OBP.toFixed(3)}}</td><td style="color:var(--text2);font-size:.72rem">${{b.PlateAppearances.toFixed(1)}}</td></tr>`;}}).join('');
}}
...
function initWeekly(){{
  ...
  if(!Object.keys(WP).length){{
    el.innerHTML='<div class="es"><div class="ic">⚾</div><div>No pitchers from your roster found in todays data. Upload multiple days for full weekly schedule.</div></div>';
    return;
  }}
